...
const RetakeCourseRegistrations = () => {
  ...
  const fetchRegistrations = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/retake-courses/read.php?status=${statusFilter}`);
      ...
      setError('Failed to load retake course registrations. Please try again later.');
      setLoading(false);
      console.error('Error fetching retake course registrations:', err);
    }
  };
  ...
  const handleApprove = async (id) => {
    try {
      setProcessing(true);
      await api.post('/retake-courses/approve.php', {
        registration_id: id,
        status: 'approved'
      });
  ...
  const handleBulkAction = async (action) => {
    ...
    if (window.confirm(`Are you sure you want to ${action} the selected retake course registrations?`)) {
      try {
        setProcessing(true);
        
        // For each selected ID, make an API call
        const promises = selectedIds.map(id => 
          api.post('/retake-courses/approve.php', {
            registration_id: id,
  ...
  const filteredRegistrations = registrations.filter(reg => 
    ...
    (reg.class_name && reg.class_name.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  if (loading) {
    return (
      <div className="text-center mt-5">
        ...
        <p className="mt-2">Loading retake course registrations...</p>
      </div>
    );
  }
        ...
        <div className="card-body">
          ...
          {currentItems.length === 0 ? (
            <div className="alert alert-info">
              {searchTerm 
                ? "No retake course registrations found matching your search criteria." 
                : `No ${statusFilter} retake course registrations available.`}
            </div>
          ) : (
                  ...
                  {currentItems.map(reg => (
                    <tr key={reg.registration_id}>
                      ...
                      <td className="text-center">
                        <span className={`badge bg-${getGradeBadge(reg.previous_grade)}`}>
                          {reg.previous_grade}
                        </span>
                      </td>
                      <td>{reg.semester} {reg.academic_year}</td>
                      <td>
...
const getGradeBadge = (grade) => {
  ...
  if (numericGrade < 60) return 'danger';
  if (numericGrade < 70) return 'warning';
  if (numericGrade < 80) return 'info';
  if (numericGrade < 90) return 'primary';
  return 'success';
};

export default RetakeCourseRegistrations;
